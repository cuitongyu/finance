...
contract POKTokenWrapper {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // 抵押币种  POK
    IERC20 public stakeToken = IERC20(
        0x0CF2ca3fB8f31d157Cc233dDA4735609ACB01a3a
    );
    ...
    function stake(uint256 amount) public {
        _totalSupply = _totalSupply.add(amount); //  总抵押
        _balances[msg.sender] = _balances[msg.sender].add(amount); // 个人余额
...
contract Seg {
    function setAmount(uint256 amount, address _address) external;
}

contract POKPool is POKTokenWrapper, Ownable {
    Seg seg;

    constructor(address _seg) public {
        seg = Seg(_seg);
    }
    ...
    // 开始时间
    uint256 public constant startTime = 1604648026; //utc+8 2020-09-09 20:00:00
    // 每秒奖励多少
    uint256 public constant rewardRate = 1157407407407; //0.1/86400
    // 上次更新时间
    ...
    function getReward() public checkStart updateReward(msg.sender) {
        ...
        if (reward > 0) {
            seg.setAmount(reward, msg.sender);
