...
contract NFT {
    ...
    mapping(address => mapping(uint256 => PokAuctionInfo))
        public historyAuctions;
...
contract DEGOERC20Pool is NFTWrapper, Ownable {
    using SafeERC20 for IERC20;

    NFT _acu;
    IERC20 token;
    address public nftAddress;
    address public acuAddress;
    // 开始时间
    ...
    constructor(address _nftToken, address _acutionToken) public {
        ...
        acuAddress = _acutionToken;
        _nft = NFT(_nftToken);
        _acu = NFT(_acutionToken);
    }
        ...
        updateReward(msg.sender)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            // 获取定价
            uint256 price = historyAuctions(tokenIds[i]);
            require(price > 0, "Nft no price");
            tokenPrice[tokenIds[i]] = price;
        ...
        updateReward(msg.sender)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            super.withdraw(tokenPrice[tokenIds[i]], tokenIds[i]);
    ...
    function historyAuctions(uint256 _tokenId) public view returns (uint256) {
        (
            ...
            uint256 price,
            uint256 currentPrice
        ) = _acu.historyAuctions(nftAddress, _tokenId);
        return price;
    }
